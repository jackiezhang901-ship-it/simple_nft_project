...
import (
	"NFTAuctionSync/service/auctionindexer"
	"context"
	"fmt"
	"sync"

	"github.com/RCCHackathonTeam2/NFTAuctionBase/chain"
	...
	"NFTAuctionSync/model"
	"NFTAuctionSync/service/collectionfilter"
	"NFTAuctionSync/service/config"
	"NFTAuctionSync/service/nft"
	"NFTAuctionSync/service/orderbookindexer"
)
...
type Service struct {
	...
	auctionIndexer   *auctionindexer.Service
...
func New(ctx context.Context, cfg *config.Config) (*Service, error) {
	...
	var auctionSyncer *auctionindexer.Service
	...
	case chain.EthChainID, chain.OptimismChainID, chain.SepoliaChainID:
		...
		auctionSyncer = auctionindexer.New(ctx, cfg, db, kvStore, chainClient, cfg.ChainCfg.ID, cfg.ChainCfg.Name, orderManager)
		nftService = nft.New(ctx, cfg, db, kvStore, chainClient, cfg.ChainCfg.ID, cfg.ChainCfg.Name, orderManager)
	}
	if err != nil {
