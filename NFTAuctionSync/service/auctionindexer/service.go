...
func (s *Service) Start() {
	threading.GoSafe(s.SyncAuctionEventLoop)
	threading.GoSafe(s.UpKeepingCollectionFloorChangeLoop)
}

func (s *Service) SyncAuctionEventLoop() {
	...
	for {
		select {
		case <-s.ctx.Done():
			xzap.WithContext(s.ctx).Info("SyncAuctionEventLoop stopped due to context cancellation")
			return
		default:
		}
		...
		for _, log := range logs { // 遍历日志，根据不同的topic处理不同的事件
			...
			case LogStartAuctionTopic:
				s.handleStartAuctionEvent(ethLog)
			case LogCancelTopic:
		...
		if err := s.db.WithContext(s.ctx).Table(base.IndexedStatusTableName()).
			...
			xzap.WithContext(s.ctx).Error("failed on update auction event sync block number",
				zap.Error(err))
			return
		}

		xzap.WithContext(s.ctx).Info("sync auction event ...",
...
// 处理发起拍卖事件
func (s *Service) handleStartAuctionEvent(log ethereumTypes.Log) {
	var event struct {
		StartPrice      *big.Int
	...
	// Unpack data
	err := s.parsedAbi.UnpackIntoInterface(&event, "StartAuctionEvent", log.Data) // 通过ABI解析日志数据
	if err != nil {
		xzap.WithContext(s.ctx).Error("Error unpacking StartAuctionEvent event:", zap.Error(err))
		return
	}

	AuctionKey := log.Topics[1].Bytes()
	TokenId := new(big.Int).SetBytes(log.Topics[2].Bytes())
	Seller := common.BytesToAddress(log.Topics[3].Bytes())

	newAuction := multi.Auction{
		TokenId:         TokenId.String(),
		Seller:          Seller.String(),
		AuctionKey:      HexPrefix + hex.EncodeToString(AuctionKey),
		AuctionType:     event.AuctionType,
		StartPrice:      decimal.NewFromBigInt(event.StartPrice, 0),
		MinBidIncrement: decimal.NewFromBigInt(event.MinBidIncrement, 0),
		StartTime:       time.Unix(int64(event.StartTime), 0),
		EndTime:         time.Unix(int64(event.EndTime), 0),
		Status:          "pending",
	}

	if err := s.db.WithContext(s.ctx).Table(multi.AuctionTableName(s.chain)).Clauses(clause.OnConflict{
		DoNothing: true,
	}).Create(&newAuction).Error; err != nil { // 将拍卖信息存入数据库
	...
	newActivity := multi.Activity{ // 将拍卖信息存入活动表
		ActivityType:      3,
		Maker:             Seller.String(),
		Taker:             ZeroAddress,
		MarketplaceID:     multi.MarketAuction,
		CollectionAddress: ZeroAddress,
		TokenId:           TokenId.String(),
		CurrencyAddress:   s.cfg.ContractCfg.EthAddress,
		Price:             decimal.NewFromBigInt(event.StartPrice, 0),
	...
	if err := s.orderManager.AddToOrderManagerQueue(&multi.Order{ // 将拍卖信息存入订单管理队列
		ExpireTime:        newAuction.EndTime.Unix(),
		OrderID:           newAuction.AuctionKey,
		CollectionAddress: ZeroAddress,
		TokenId:           TokenId.String(),
		Price:             decimal.NewFromBigInt(event.StartPrice, 0),
		Maker:             Seller.String(),
	}); err != nil {
		xzap.WithContext(s.ctx).Error("failed on add auction to manager queue",
			zap.Error(err),
			zap.String("order_id", newAuction.AuctionKey))
	}
}
